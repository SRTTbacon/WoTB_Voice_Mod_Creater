...
namespace WoTB_Voice_Mod_Creater.Wwise_Class.BNK_To_Wwise_Project
{
    ...
    public class Actor_Mixer
    {
        ...
        public async Task Get_Actor_Mixer_Hierarchy(string To_File, System.Windows.Controls.TextBlock TextBlock)
        {
            Message_T = TextBlock;
                ...
                while (Not_Done_Number.Count > 0)
                {
                    ...
                    foreach (int Number in Not_Done_Number)
                    {
                        ...
                        foreach (Actor_Mixer_Info Temp_Relation in BNK_Info.Actor_Mixer_Child_Line)
                        {
                            ...
                            {
                                Set_Actor_Mixer_Container(BNK_Info.Relation[Number]);
        ...
        void Set_Actor_Mixer_Container(BNK_Relation Temp_Relation)
        {
            ...
            else if (Type == "CAkSound")
            {
                ...
                if (Temp_Line_02 == -1)
                    return;
                Add_CAkSound(Temp_Relation, Temp_Line_02, To_Line);
            }
        }
        //Blend Containerを設定
        ...
        //Soundを設定
        void Add_CAkSound(BNK_Relation Temp_Relation, int From_Line, int To_Line)
        {
        ...
        //初期値を記入
        void Add_PropertyList(BNK_Relation Temp_Relation, string Name, string Value, Property_Type Type, bool IsIntMode)
        {
            int Line_Start = -1;
            for (int Number = 5; Number < BNK_Info.Actor_Mixer_Hierarchy_Project.Count; Number++)
            {
